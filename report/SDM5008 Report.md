...
1. **Encoder-MLP**：基础盲视策略，通过本体感知信息估计机器人基座线速度。机器人没有外部感知能力，仅靠本体感知（IMU、关节）行走。

   <img src="images/SDM5008 Report/image-20260105171415652.png" alt="image-20260105171415652" style="zoom:25%;" />

2. **HIM (Hybrid Internal Model)** ：HIM 训练一个基于本体感知历史的**内部模型 (Internal Model/Estimator)**，通过**对比学习**显式地**预测**隐式特权信息，使得 Policy 不仅仅是被 Critic “指导”，而是自身具备了从本体感知中**推理**环境和自身状态的能力。

   <img src="images/SDM5008 Report/image-20260105170815865.png" alt="image-20260105170815865" style="zoom:70%;" />

3. **PIM (Perceptive Internal Model)**：在 HIM 的 Policy 观测量中引入了视觉（高程图）编码器，构建了一个**多模态的内部模型**。与简单的叠加输入不同，PIM 利用雷达扫描信息来**修正和增强**对环境状态的估计（即构建包含几何信息的内部表征），从而让机器人能够**主动规划落点**以应对盲视无法处理的**剧烈地形变化**（如陡峭楼梯或断崖）。
...
| -------- | ----------- | ----------------------- | ---------- |
| **Policy 输入** | **纯本体感知** (关节位置/速度, IMU, 指令) | **本体感知** (同 Base) | **本体感知 + 外部感知（地形高度扫描）** |
| **Critic 输入** | 本体感知 + 特权物理信息 (摩擦力, 质量等)  | 本体感知 + 特权物理信息 + **地形高度扫描 (Heights)** | 本体感知 + 特权物理信息 + **地形高度扫描 (Heights)** |
...
下表总结了三种配置在 `limx_pointfoot_env_cfg.py` 中的具体权重差异。
...
| **pen_flat_orientation** | 姿态惩罚 (保持水平)    | -10.0 | **-2.0** | **-0.2** | PIM 放宽了姿态约束，高程图信息使机器人能够更加自然适应复杂地形，无需大惩罚。 |
| **pen_base_height**      | 基座高度维持           | -20.0 | **-1.0** | **-1.0** | 降低惩罚权重，使机器人在高度变化剧烈的地形中能够有效调整姿态。 |
| **rew_feet_clearance**   | 足部抬高奖励           | N/A (Default 0) | **0.2** | **0.5** | HIM和PIM均奖励抬脚高度，以应对复杂的地形。 |
| **test_gait_reward**     | 步态约束奖励           | 1.0 | **0.5** | **0.4** | 在复杂地形上，严格的强制步态可能适得其反，因此降低了步态约束的权重。 |
| **pen_feet_distance**    | 双脚距离惩罚           | -10.0 | **-40.0** | **-40.0** | 增大惩罚，防止机器人因为复杂地形而损失步态风格。 |
| **移除的项**             | 精简奖励函数           | N/A | `feet_regulation`, `landing_vel` | `feet_regulation`, `landing_vel` | HIM/PIM 移除了针对平地优化的着陆速度和足部调节规则，依靠状态预测算法自动调节。 |
...
| -------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **算法架构**               | Base Blind (Encoder-MLP)                                     | 仅观测本体感知信息 (Proprioception)                          |
...
由于采用了“盲视”（Blind）设置，观测空间被精简为纯本体感知信息。观测向量 $\mathbf{o}_t$ 包含以下关键分量：
...
* **振荡幅度与有界性 (Amplitude & Boundedness)**
  ...
  - **Roll (横滚角)**：波动范围相对较小（Range：0.354 rad，约 $20.3^\circ$）。考虑到双足机器人的横向平衡通常较难维持，这一波动幅度表明机器人在高速运动中左右摇摆较为剧烈，呈现出“踏步调整”的策略来维持平衡。

* **稳态偏置 (Steady-State Bias)**

  观察零基准线（Zero Ref），两个自由度均存在微小的非零均值偏置：

  - **Roll**：呈现靠近0的微小负偏置，机器人能够基本保持基座水平。
  - **Pitch**：主体位于正值区间，同样靠近0参考线。表明机器人在运动时保持着轻微的**“前倾”姿态**。这对于较高速度的运动是合理的，有助于质心前移以辅助加速。
